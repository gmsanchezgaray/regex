<div class="container-fluid row d-flex justify-content-center align-items-center ">
    <div class="col-md-10 card mb-3 p-md-4">
        <div class="row g-0">
            <div class="col-md-3 align-self-center text-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="80" height="80" fill="currentColor" class="bi bi-cart-x" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .491.592l-1.5 8A.5.5 0 0 1 13 12H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l1.313 7h8.17l1.313-7H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 0 0 2 1 1 0 0 0 0-2zm7 0a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
                    <path fill-rule="evenodd" d="M6.646 5.646a.5.5 0 0 1 .708 0L8.5 6.793l1.146-1.147a.5.5 0 0 1 .708.708L9.207 7.5l1.147 1.146a.5.5 0 0 1-.708.708L8.5 8.207 7.354 9.354a.5.5 0 1 1-.708-.708L7.793 7.5 6.646 6.354a.5.5 0 0 1 0-.708z"/>
                </svg>
            </div>
            <div class="col-md-8">
                <div class="card-body">
                    <h4 class="card-title">No hay publicaciones que coincidan con tu búsqueda.</h4>
                    <ul>
                        <li><b>Revisá la ortografía</b> de la palabra.</li>
                        <li>Utilizá <b>palabras más genéricas</b> o menos palabras.</li>
                        <li><a href="javascript:history.back()">Navegá por las categorías</a> para encontrar un producto similar</li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</div>