...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errores !== '') {
        ...
        $marca = Marca::updateMarca(ucfirst($nombre),$sitio_web,$telefono,$observaciones);

        if(Marca::getCount() > 0){

            /// The Brand was updated in the database
            $mensajeUsuario = getAlert('Se modifico la marca de manera exitosa.', 'alert-success');
        }else{

            ///If there was a problem in the database
            $mensajeUsuario = getAlert(' No se pudo modificar la marca del registro de la base de datos.', 'alert-danger');
        }
    ...
    <div class="container row-cols-1 my-5">
        <div class="card container col-md-8 px-0">
            ...
            <div class="card-body">
                ...
                <h4>Id de la marca a modificar: <span class="text-info"><?= $marca->id_marca ?></span></h4>

                <form action="" method="post" enctype="multipart/form-data">
                    <div class="form-row">
                        <div class="form-group col-lg-6">
                            <label for="nombre">Nombre <span class="small text-danger">(*)</span></label>
                            <input type="text" class="form-control" id="nombre" name="nombre" autofocus value="<?= $marca->nombre ?>">
                        </div>
                        <div class="form-group col-lg-6">
                            <label for="sitio_web">Sitio Web <span class="small text-danger">(*)</span></label>
                            <input type="text" class="form-control" id="sitio_web" name="sitio_web" value="<?= $marca->sitio_web?>">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-lg-6">
                            <label for="telefono">Télefono <span class="small text-danger">(*)</span></label>
                            <input type="text" class="form-control" id="telefono" name="telefono" value="<?= $marca->telefono?>">
                        </div>
                        <div class="form-group col-lg-6">
                            <label  for="observaciones">Observaciones</label>
                            <div>
                                <textarea class="w-100 form-control" name="observaciones" id="observaciones"  rows="1"><?= $marca->observaciones?> </textarea>
                            </div>
                        </div>
                    </div>
                    <div>
                        <button type="submit" name="actualizar" class="btn btn-primary">Editar</button>
                        <a class="btn btn-outline-primary" href="marcas_listado.php">Cancelar</a>
                    </div>
                </form>
                <?php
                }
